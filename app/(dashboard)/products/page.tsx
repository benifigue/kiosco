    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '13px' }}>{filtered.length} productos</p>
        </div>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td style={{ fontFamily: 'var(--font-mono)', fontSize: '12px' }}>{p.barcode ?? '—'}</td>
                    <td style={{ fontFamily: 'var(--font-mono)', fontSize: '12px' }}>{p.internalCode ?? '—'}</td>
                    ...
                    <td>
                      <span className={`badge badge-${stockStatus}`}>
                        {p.stock === 0 ? '⚠ Sin stock' : p.stock <= p.minStock ? `⚡ ${p.stock}` : p.stock}
                      </span>
                    </td>
                    <td>
                    ...
                    {userRole === 'ADMIN' && (
                      <td>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button className="btn btn-ghost" style={{ padding: '4px 10px', fontSize: '12px' }} onClick={() => openEdit(p)}>
                            Editar
                          </button>
                          <button
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  ...
                  <input className="input" value={form.internalCode} onChange={(e) => setForm((f) => ({ ...f, internalCode: e.target.value }))} />
                </div>
                <div>
                  <label className="label">Precio Compra *</label>
                  <input className="input" type="number" step="0.01" min="0" required value={form.purchasePrice} onChange={(e) => setForm((f) => ({ ...f, purchasePrice: e.target.value }))} />
                </div>
                <div>
                  <label className="label">Precio Venta *</label>
                  <input className="input" type="number" step="0.01" min="0" required value={form.salePrice} onChange={(e) => setForm((f) => ({ ...f, salePrice: e.target.value }))} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input className="input" type="number" min="0" required value={form.minStock} onChange={(e) => setForm((f) => ({ ...f, minStock: e.target.value }))} />
                </div>
              </div>

              {/* Margin preview */}
              {form.salePrice && form.purchasePrice && (
                <div style={{ marginTop: '12px', padding: '10px 14px', background: 'var(--bg)', borderRadius: '8px', fontSize: '13px' }}>
                  Ganancia por unidad: {formatCurrency(Number(form.salePrice) - Number(form.purchasePrice))} &nbsp;
                  ({Number(form.salePrice) > 0 ? ((Number(form.salePrice) - Number(form.purchasePrice)) / Number(form.salePrice) * 100).toFixed(1) : 0}% margen)
                </div>
              )}
